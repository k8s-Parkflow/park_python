...
- 요청 조건, 응답 형식, 오류 계약을 한 번에 확인할 수 있도록 정리한다.
...
- `availableCount`는 전체 Zone 기준 합산값이다.
...
- Acceptance Test 검증 완료
- Contract Test 검증 완료
- Repository Test 검증 완료

검증 명령:

```bash
./.venv/bin/python manage.py test acceptance contract repository --settings=park_py.settings_test
```
